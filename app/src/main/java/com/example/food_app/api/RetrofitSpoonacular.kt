...
//        const val API_KEY = "da56f65f2b7945b3ac1425493af40362"
    ...
    inner class Cache {
        ...
        init {
            CoroutineScope(Dispatchers.Default).launch{
                fireBaseViewModel.getFav { favCache = it }
            }
        }
        ...
        private fun updateCache(recipe : ExtendedRecipe){
            recipeCache.find { it.id == recipe.id }
                .apply{ recipeCache[(recipeCache.indexOf(this))] = recipe }?: recipeCache.add(recipe)
        }
        fun getFavourite() = favCache
    ...
    override suspend fun getRandomRecipes(number: Int): List<ExtendedRecipe>? = recipeApi.getRandomRecipe(API_KEY, number).recipes?.map { it }
    ...
    fun getFavourite(): List<ExtendedRecipe> = cache.getFavourite()
